...
const PaginaVendas = () => {
  ...
  const handleBulkMarkAsPaid = async () => {
    ...
    try {
      ...
      await Promise.all(
        selectedVendas.map((id) => updateStatusPagamentoVenda(id, "Pago"))
      );
      setVendas((prevVendas) =>
        prevVendas.map((v) =>
          selectedVendas.includes(v.id) ? { ...v, statusPagamento: "Pago" } : v
        )
      );
      setSelectedVendas([]);
      toast.dismiss();
  ...
  const handleUpdateStatus = useCallback(
    async (vendaId, statusAtual, novoStatus) => {
      try {
        await updateStatusPagamentoVenda(vendaId, novoStatus);
        setVendas((vendasAtuais) =>
          vendasAtuais.map((v) =>
            v.id === vendaId ? { ...v, statusPagamento: novoStatus } : v
          )
        );
  ...
  const filteredAndSortedVendas = useMemo(() => {
    let vendasFiltradas = [...vendas].filter((venda) => {
      ...
      const matchStatus =
        filtroStatus === "todos" || venda.statusPagamento === filtroStatus;

      const matchFormaPagamento =
        filtroFormaPagamento === "todos" ||
        (venda.formaPagamento &&
          venda.formaPagamento.includes(filtroFormaPagamento)) ||
        (venda.pagamentos &&
          venda.pagamentos.some((p) => p.metodo === filtroFormaPagamento));
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        sortable: true,
        renderCell: (item) => {
          // VERIFICAÇÃO NOVA: Se houver lista de pagamentos, detalha cada um
          if (
            item.pagamentos &&
            Array.isArray(item.pagamentos) &&
          ...
          // Fallback para vendas antigas ou simples
      ...
      },
 {
        ...
        sortable: true,
        renderCell: (item) => {
            // ... (Mantenha o código original desta coluna)
      ...
      },
      {
        ...
        renderCell: (item) => (
          <div className="flex items-center gap-4 text-gray-500 text-lg">
            <button
              onClick={() => handleOpenComprovanteModal(item)}
            ...
            {item.statusPagamento !== "Cancelado" && (
              <>
                ...
                <button
                  onClick={() => handleOpenCancelarModal(item)}
